...
{
	 wife->shared_print(wife->ID(), ": Time to do some more housework!"); 
...
void DoHouseWork::Execute(MinersWife* wife)
{
  ...
  case 0:
	...
	wife->shared_print(wife->ID(), ": Moppin' the floor!"); 
    break;

  case 1:

	wife->shared_print(wife->ID(), ": Washin' the dishes"); 
    break;

  case 2:

	wife->shared_print(wife->ID(), ": Makin' the bed"); 
    break;
  }
}
...
{  
  wife->shared_print(wife->ID(), ": Walkin' to the can. Need to powda mah pretty li'lle nose"); 
...
{
  wife->shared_print(wife->ID(), ": Ahhhhhh! Sweet relief!");
  wife->GetFSM()->RevertToPreviousState();
}

void VisitBathroom::Exit(MinersWife* wife)
{
	 wife->shared_print(wife->ID(), ": Leavin' the Jon");
}
...
void CookStew::Enter(MinersWife* wife)
{
  ...
  if (!wife->Cooking())
  {
	wife->shared_print(wife->ID(), ": Putting the stew in the oven");
...
{
  wife->shared_print(wife->ID(), ": Fussin' over food");
}
